...
async function page({ params }: { params: { id: string } }) {
  ...
  const creatorInfo = await api.creator.getCreator(creatorID);
  const creatorVideos = api.creator.getCreatorVideos(creatorID);
  ...
  return (
    <div className="relative flex h-full flex-col items-center overflow-x-hidden">
      ...
      <p className="max-w-[60ch] whitespace-pre-line text-center">{creatorInfo.biography}</p>
      <div className="my-10 flex h-56 items-center gap-2">
        <p> Visit Channel</p>
        <a href={`//www.youtube.com/@${creatorInfo.youtubeHandle}`} target="_blank">
